...
echo "  talk2stindex-mcp sse --port 8016    # Start MCP server"
echo ""

if python -c "import talk2stindex.mcp" 2>/dev/null; then
    echo_info "MCP Server commands:"
    echo "  talk2stindex-mcp sse --config config.mcp.yml --host 127.0.0.1 --port 8016"
    echo "  talk2stindex-mcp sse --port 8016  # Custom port"
    echo ""
